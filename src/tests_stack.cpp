...
TEST(s21_stack_test, constr) {
  ...
  stack<test_obj, list<test_obj>> stack_c(
      stack<test_obj, list<test_obj>>{arr[0], arr[1], arr[2]});
  EXPECT_EQ(stack_c.top().id, arr[2].id);
  EXPECT_EQ(stack_c.size(), 3);
  EXPECT_EQ(stack_c.empty(), false);
  stack<test_obj, list<test_obj>> stack_d{
      std::move(stack<test_obj, list<test_obj>>{arr[0], arr[1], arr[2]})};
