...
TEST(StackTest, constr) {
  ...
  s21::stack<test_obj, s21::list<test_obj>> stack_d{
      std::move(s21::stack<test_obj, s21::list<test_obj>>{arr[0], arr[1], arr[2]})};
  ...
  stack_f = std::move(s21::stack<test_obj, s21::list<test_obj>>{arr[3], arr[4], arr[5]});
...
TEST(StackTest, swap) {
  ...
  EXPECT_EQ(stack_a.top().id, arr[2].id);
  EXPECT_EQ(stack_a.size(), 3);
  s21::stack<test_obj, s21::list<test_obj>> stack_b{arr[3], arr[4], arr[5], arr[6]};
