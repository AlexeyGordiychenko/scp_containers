...
TEST(StackTest, constr) {
  ...
  // default constructor
  scp::stack<test_obj, scp::list<test_obj>> stack_a;
  EXPECT_EQ(stack_a.size(), 0);
  EXPECT_EQ(stack_a.empty(), true);

  // initializer list constructor
  scp::stack<test_obj, scp::list<test_obj>> stack_b{arr[0], arr[1], arr[2]};
  EXPECT_EQ(stack_b.top().id, arr[2].id);
  EXPECT_EQ(stack_b.size(), 3);
  EXPECT_EQ(stack_b.empty(), false);

  // copy constructor
  scp::stack<test_obj, scp::list<test_obj>> stack_g{arr[0], arr[1], arr[2]};
  scp::stack<test_obj, scp::list<test_obj>> stack_c(stack_g);
  EXPECT_EQ(stack_c.top().id, arr[2].id);
  EXPECT_EQ(stack_c.size(), 3);
  EXPECT_EQ(stack_c.empty(), false);

  // move constructor
  scp::stack<test_obj, scp::list<test_obj>> stack_d{std::move(
      scp::stack<test_obj, scp::list<test_obj>>{arr[0], arr[1], arr[2]})};
  EXPECT_EQ(stack_d.top().id, arr[2].id);
  EXPECT_EQ(stack_d.size(), 3);
  EXPECT_EQ(stack_d.empty(), false);

  // move assign operator
  scp::stack<test_obj, scp::list<test_obj>> stack_e{arr[3], arr[4], arr[5]};
  scp::stack<test_obj, scp::list<test_obj>> stack_f{arr[0], arr[1], arr[2]};
  stack_f = std::move(
      scp::stack<test_obj, scp::list<test_obj>>{arr[3], arr[4], arr[5]});
...
TEST(StackTest, swap) {
  test_obj arr[7];
  scp::stack<test_obj, scp::list<test_obj>> stack_a{arr[0], arr[1], arr[2]};
  EXPECT_EQ(stack_a.top().id, arr[2].id);
  EXPECT_EQ(stack_a.size(), 3);
  scp::stack<test_obj, scp::list<test_obj>> stack_b{arr[3], arr[4], arr[5],
                                                    arr[6]};
...
TEST(StackTest, insert_many_front) {
  scp::stack<test_obj, scp::list<test_obj>> s;
  test_obj arr[6];
